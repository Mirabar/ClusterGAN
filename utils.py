...
def e_loss(true, pred, n_classes=10, bc=10, bn=10, combined_loss=True):
    ...
    zc_logits = softmax(ezc) #get probabilities for ce
...
def zc_acc(pred, true):
    '''
    :param pred: predicted zc
    :param true: true labels
    :return: classification evaluation after applying softmax to zc
    '''
    pred_prob = softmax(tf.convert_to_tensor(pred))
    pred_label = np.argmax(pred_prob, axis=-1)

    class_acc, all_acc = cluster_acc(true, pred_label)

    ari = adjusted_rand_score(true, pred_label)
    nmi = normalized_mutual_info_score(true, pred_label)

    return class_acc, all_acc, ari, nmi
